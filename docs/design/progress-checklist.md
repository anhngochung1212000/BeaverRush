...
- [x] 🆕 Concept-match polish (2026-07-04, theo concept art): nước tint teal đậm · lane đứt nét trắng + nhãn LANE 1-5 · mũi tên trắng đặc 1 hàng ngay trên đập · bọt trắng chân đập · bờ rừng rậm hơn (cây dày + khúc gỗ nằm) · progress bar trại xanh lá bo tròn · KÍCH THƯỚC khúc gỗ/block/đá bẹt theo tỉ lệ bó gỗ 1.5:1 (`cellH` 150→100, originY suy lại — đáy đập vẫn 1500; gỗ trôi = ĐÚNG 1 ô grid 140×100 (trùng cỡ block khi neo), đá 126×84 vừa lane) · CONTENT_CROP (drawSprite): crop bỏ viền trong suốt/bóng của wood.png (crop lõi vân gỗ = khối CHỮ NHẬT, không còn hình thang) + rock.png → block/gỗ/đá fill ĐẦY ô, đập gỗ liền mạch
- [x] 🆕 Art NƯỚC + THÁC thật (2026-07-04, từ `waterfall.png` sprite sheet): cắt `river_flow.png` (330², mặt sông TILE GƯƠNG chống seam + cuộn) → bg_river + fill thanh nước UI; `fall_streaks.png` (streak thuần) → con thác TILE GƯƠNG + cuộn xuống 700px/s×flowFactor (chảy mạnh) + `fall_lip.png` nối sông↔thác (nước tràn qua mép) + BỌT TRẮNG động chân thác (churn + bong bóng thủ tục); tint phủ cả sông+thác cùng tông; đá bờ kéo sát nước (Ground anchor 0.76, overhang 0).
  - 🆕 2026-07-04 (cuối): 3 crop river_flow/fall_streaks/fall_lip tái tạo từ MỘT ảnh `river.png` (nửa trên sông, nửa dưới thác, vạch lip bọt ở giữa) → sông và thác CÙNG NGUỒN nên khớp tông/kiểu tuyệt đối. `water.png`/`waterfall.png`/`waterfall_big.png` giờ KHÔNG còn tham chiếu (giữ file, không xoá). Bỏ `WATERFALL_FLOW_PX_PER_SEC`. `water.png` cũ không còn tham chiếu (giữ file)
- [x] 🆕 Căn scene khớp concept (2026-07-04, theo spec user): (#4) lane chỉ còn 4 đường dọc nét đứt mảnh, bỏ lưới ô ngang, mũi tên trắng đậm ngay trên đập · (#5) gradient tối dần phía xa (chiều sâu sông) · (#2) thác chỉ-nước, bờ khung liền không lệch đá · (#1 INTERIM) overlay tối 2 bờ về tông canyon — ⏳ vách đá canyon tối thật cần asset mới (#7, user tạo qua GPT)
