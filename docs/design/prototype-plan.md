# BEAVERRUSH — Prototype Plan (HTML5, v2.3)

> **Changelog v2.3 (2026-07-03) — REFRAME LAYOUT NGANG (theo ref art):** amend v2.0 điểm (1) camera pan. **SÔNG HẸP giữa + 2 BỜ RỪNG vừa TRONG 1 KHUNG HÌNH — BỎ camera pan.** `Camera`: `forestW` = bề rộng mỗi bờ trong khung (175); `riverW = CANVAS_W − 2·forestW` (730); `worldW = CANVAS_W`, `panLimit = 0` (Renderer translate cố định = forestW, bờ trái lộ tại screen 0). Grid canh giữa sông: `cellW` 180→**140**, `originX` 90→**15** (screen 190→890). 🆕 2026-07-04 (concept-match): `cellH` 150→**100** — block BẸT theo tỉ lệ bó gỗ wood.png (1.5:1), các hàng đập khít; originY suy lại theo luật đáy đập 1500 (lv0 **1300**, lv1 **1100**); gỗ trôi vẽ ĐÚNG 1 ô grid (cellW×cellH = 140×100, trùng cỡ block khi neo — chốt 2026-07-04), đá 126×84 (vừa lane). Background sông + thác vẽ theo `riverW` (thác phủ TRỌN bề rộng sông như ref); bờ phải neo tại river-x = riverW. `originY`/`cellH`/đáy đập 1500 GIỮ NGUYÊN (v2.2). Drag nền vô hiệu (panLimit 0) — chỉ còn tap = lệnh.

> **Changelog v2.2 (2026-07-03) — V3.2 PATCH (travel time · dòng chảy theo nước · rock targeting · nhặt block đang đứng · layout 1500):** amend v2.1 — chỗ nào mâu thuẫn, v2.2 THẮNG. (1) **TRAVEL TIME thay px/s:** bỏ `floatingLog.speedPx`/`rockFallSpeedPx`/`logFallSpeedPx` — mỗi level config `drift: {travelSecStart, travelSecMin}` (Thẳng 8→4s, Chữ U 6.4→3.2s); quãng chuẩn = spawnY → tâm ô neo row 0; **vận tốc MỌI vật trôi trên sông = quãng chuẩn ÷ travelSec** (`world.driftSpeedPx` — SUY RA, dùng chung gỗ thả + obstacle, vật đang trôi giữa sông cũng đổi tốc theo). (2) **DÒNG CHẢY THEO MỰC NƯỚC:** nước dâng → `travelSec` RÚT NGẮN tuyến tính từ Start tới sàn Min (nước đầy); visual texture nước cuộn theo `flowFactor = start/travelSec` (tích phân `flowTime`, tránh texture nhảy). (3) **ROCK TARGETING:** obstacle PHÁ HỦY tự nhắm lane NHIỀU BLOCK NHẤT (grid + ô ảo), lane KHÓA lúc telegraph (cảnh báo trung thực); hòa → lane index nhỏ; sông trống → lane schedule (fallback, trôi vô hại); GỖ vẫn theo lane data. LOG obstacle spawn = `FloatingLog` (chung 100% cơ chế gỗ thả); ROCK DAMAGE=1 (phá 1 block rồi vỡ, ⏳ N block sau); state schedule nằm trong `world.obstacleState` (restart tự sạch). (4) **NHẶT BLOCK ĐANG ĐỨNG (v3.2 GDD):** FETCH chính ô đang đứng hợp lệ — hải ly tự NÉ sang block kề rồi nhặt (`pathToAdjacent(C,C)` trả path né sẵn); đảo 1 ô → từ chối. (5) **LAYOUT:** đáy đập CỐ ĐỊNH y=1500 mọi level (`originY = 1500 − rows·cellH` — level 0: 1200), thác 1500→1920. (6) **END SCREEN (M3):** nút Chơi lại (restart tại chỗ qua `loadLevel`) + Màn tiếp khi WIN (level cuối → chơi lại); WIN/LOSE khóa TOÀN BỘ input trừ nút overlay (chống race: chốt state lúc pointerdown; `inputCooldown` 0.3s sau restart chặn double-tap), camera tự kéo mượt về `x = forestW`. (7) **JUICE/SFX (M5):** layer `fx.js` (smash/splash/pop/burst) + rung màn + pulse thanh nước ≥80% + fade-in 0.4s vào level + hook `core/Sfx.js` 11 event (log verbose, map file thật vào `FILES`); texture nước thật `water.png` dùng chung bg_river/waterfall/ui_water_fill (tile + cuộn, AssetStore dedupe theo path).
...
- Kho = **1 nút UI cố định** (icon gỗ + bộ đếm `count`, KHÔNG giới hạn max). Tap nút khi `count > 0` (không phụ thuộc trạng thái hải ly):
  ...
  2. Spawn `FloatingLog{ lane: randInt(0..4), y: spawnY }` trôi từ trên xuống — 🆕 v2.2: KHÔNG còn speedPx per-log, vận tốc là THUỘC TÍNH CỦA SÔNG (`world.driftSpeedPx` suy từ `drift.travelSec` §D6). **Gỗ đang trôi KHÔNG tap được** (v2.1 — chỉ block đã tồn tại trên grid/ô ảo mới click-nhặt được).
- `FloatingLogSystem` mỗi tick với log DRIFTING: `y += world.driftSpeedPx*dt` (v2.2); xét lane của log:
...
- **Vùng grid (nơi đập nằm):** đặt ở giữa-dưới vùng sông để chừa không gian obstacle trôi từ trên.
  - Cell size (v2.3 concept-match): **140 × 100 px** → grid rộng 5×140 = **700 px** (căn giữa sông 730, lề 15 px mỗi bên), cao `rows`×100 (level 1: **400 px**; level 0: **200 px**). Block bẹt theo tỉ lệ bó gỗ 1.5:1. *(prototype default, tunable)*
  - 🆕 v2.2: **ĐÁY ĐẬP CỐ ĐỊNH Y = 1500** mọi level → `originY = 1500 − rows×150` (level 0 "Thẳng" = **1200**, level 1 "Chữ U" = **900**). *(chốt theo playtest 2026-07-03 — thác không chiếm nửa màn ở level ít hàng)*
  - 🆕 v2.3: **NGANG — sông hẹp giữa, 2 bờ vừa 1 khung** (bỏ pan): `forestW = 175` (bờ), `riverW = 730` (= 1080 − 2·175); grid `cellW = 140`, `originX = 15` (canh giữa sông) → grid screen-x **190→890**, bờ trái screen **0→175**, bờ phải **905→1080**.
  - Vùng trên (Y 0 → `originY`): làn obstacle + gỗ nổi trôi + telegraph. Vùng dưới (Y 1500 → 1920): thác nước (visual, phủ trọn bề rộng sông).
...
- **Kho = BỘ ĐẾM trên UI button** (nút cố định, không pan): hiển thị `count`; **tap = thả 1 khúc gỗ** (`count -= 1`) vào **lane ngẫu nhiên** — trôi theo vận tốc sông (`drift` §D6); bị chặn → **neo thành Block hoàn chỉnh** (auto slot-fill nếu trúng slot; lên ô ảo nếu lane kín), lane HOÀN TOÀN hở → mất (§A4.1). `count == 0` → nút disable, KHÔNG phải thua.
- **Gỗ nổi (`floatingLog`):** 🆕 v2.2 chỉ còn `spawnY = −150` — tốc độ trôi KHÔNG config px/s nữa, suy từ `drift.travelSec` (§D6, chung mọi vật trôi). *(prototype default, tunable)*
...
- **Tốc độ trôi — 🆕 v2.2 TRAVEL TIME (bỏ hẳn `rockFallSpeedPx`/`logFallSpeedPx`):** vận tốc là THUỘC TÍNH CỦA SÔNG, dùng CHUNG mọi vật trôi (gỗ thả + đá + gỗ obstacle): mỗi level config `drift: {travelSecStart, travelSecMin}` = thời gian trôi spawn → tâm ô neo row 0; **nước dâng → travelSec rút ngắn TUYẾN TÍNH** từ Start (nước cạn) tới sàn Min (nước đầy); `world.driftSpeedPx = quãng chuẩn ÷ travelSec` (WaterSystem tính mỗi tick — vật đang trôi giữa sông cũng tăng tốc theo). Data: Thẳng **8→4s**, Chữ U **6.4→3.2s**. *(prototype default, tunable — đã x2 một lần theo playtest)*
- **Va chạm** (block front = block sống có **row nhỏ nhất** trong lane — §A8):
  - **ĐÁ** trúng block front (kể cả block ô ảo — bị va TRƯỚC) → **kéo văng khối** xuống thác (block mất, xây lại được), rồi đá biến mất (**DAMAGE=1**; ⏳ đá phá xuyên N block — số liệu để sau); hệ quả mạng/island (§A8.1); **phá block hải ly đang đứng → hải ly rơi nước → LOSE ngay**. Lane hoàn toàn hở → trôi qua vô hại.
  - 🆕 v2.2 **ROCK TARGETING:** đá tự nhắm **lane có NHIỀU BLOCK NHẤT** (grid + ô ảo) — `lane` trong schedule chỉ là FALLBACK khi mọi lane trống; lane mục tiêu **CHỌN + KHÓA ngay lúc telegraph** (cảnh báo trung thực — block mọc thêm trong 1.2s lead không đổi mục tiêu); hòa → lane index nhỏ (deterministic). GỖ obstacle vẫn theo đúng lane data.
